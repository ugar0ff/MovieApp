...
export const createSessionApi = async (
  ...
  const loginRes = await fetch(
    `${Config.TMDB_DB}authentication/token/validate_with_login?api_key=${Config.TMDB_API_KEY}`,
    {
      method: 'POST',
  ...
  const sessionRes = await fetch(
    `${Config.TMDB_DB}authentication/session/new?api_key=${Config.TMDB_API_KEY}`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
