...
const excludedProps = new Set(['flex', 'flexGrow', 'scale', 'opacity', 'zIndex', 'angle'])

const getScaleCoefficient = () =>
  Math.min(Dimensions.get('window').width, Dimensions.get('window').height) / TARGET_DEVICE.width

const scaleValue = (value: number, coefficient = getScaleCoefficient()): number => {
    ...
    (acc, [key, value]) => {
      ...
      if (Array.isArray(value)) {
        scaledValue = value.map(item =>
          typeof item === 'number'
            ? scaleValue(item, coefficient)
            : scale(item as Record<string, unknown>, coefficient),
        )
